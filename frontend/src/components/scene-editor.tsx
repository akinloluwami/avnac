  ...
  function SceneEditor(
    ...
    useEffect(() => {
      ...
      const onDown = (e: MouseEvent) => {
        if (backgroundPopoverAnchorRef.current?.contains(e.target as Node))
          return;
    ...
    const fitZoom = useCallback(() => {
      ...
      const pct = Math.max(
        ...
        Math.min(
          ...
          Math.floor(
            Math.min(
              availW / Math.max(1, artboardW),
              availH / Math.max(1, artboardH)
            ) * 100
          )
        )
      );
      const fitScale = pct / 100;
      setPanX((viewport.clientWidth - artboardW * fitScale) / 2);
      setPanY((viewport.clientHeight - artboardH * fitScale) / 2);
      setZoomPercent(pct);
    }, [artboardH, artboardW]);
    ...
    const selectionOutlineStrokeWidth = useMemo(() => {
      const targets = selectedObjects.filter((obj) =>
        objectSupportsOutlineStroke(obj)
      );
    ...
    const elementToolbarAlignAlready = useMemo(() => {
      ...
      return artboardAlignAlreadySatisfied(
        selectionBounds,
        artboardW,
        artboardH
      );
    ...
    const elementToolbarLockedDisplay = useMemo(
      () =>
        ...
        selectedObjects.every((obj) => obj.locked),
      [selectedObjects]
    );
    ...
    const addObjects = useCallback(
      (objectsToAdd: SceneObject[]) => {
        setDoc((prev) => ({
          ...prev,
          objects: [...prev.objects, ...objectsToAdd],
        }));
    ...
    const { boardDocs: vectorBoardDocs, placeVectorBoard } =
      vectorBoardControls;
    ...
    const placeImageObject = useCallback(
      async (
        ...
        if (!opts?.width && !opts?.height) {
          ...
          height = Math.round(
            (meta.naturalHeight / meta.naturalWidth) * opts.width
          );
        } else if (!opts?.width && opts?.height) {
          width = Math.round(
            (meta.naturalWidth / meta.naturalHeight) * opts.height
          );
        }
    ...
    const pasteFromClipboard = useCallback(
      async (anchor?: { x: number; y: number }) => {
        ...
        if (
          ...
          await placeImageObject(
            text.trim(),
            anchor ? { ...anchor, origin: "top-left" } : undefined
          );
        }
      },
    ...
    const groupSelection = useCallback(() => {
      ...
      if (!group) return;
      const firstIndex = doc.objects.findIndex((obj) =>
        selectedIds.includes(obj.id)
      );
      const remaining = doc.objects.filter(
        (obj) => !selectedIds.includes(obj.id)
      );
      remaining.splice(
        firstIndex < 0 ? remaining.length : firstIndex,
        0,
        group
      );
    ...
    const ungroupSelection = useCallback(() => {
      ...
      setDoc((prev) => {
        const idx = prev.objects.findIndex(
          (obj) => obj.id === selectedSingle.id
        );
      ...
      (paint: BgValue) => {
        updateSelectedObjects((obj) => {
          if (obj.type === "line" || obj.type === "arrow")
            return setObjectStroke(obj, paint);
      ...
      (patch: Partial<TextFormatToolbarValues>) => {
        ...
        updateSelectedObjects((obj) => {
          ...
          if (patch.italic !== undefined)
            next.fontStyle = patch.italic ? "italic" : "normal";
    ...
    const alignElementToArtboard = useCallback(
      (kind: CanvasAlignKind) => {
        ...
        if (kind === "centerH")
          dx =
            artboardW / 2 - (selectionBounds.left + selectionBounds.width / 2);
        if (kind === "right")
          dx =
            artboardW -
            ARTBOARD_ALIGN_PAD -
        ...
        if (kind === "centerV")
          dy =
            artboardH / 2 - (selectionBounds.top + selectionBounds.height / 2);
        if (kind === "bottom")
          dy =
            artboardH -
            ARTBOARD_ALIGN_PAD -
    ...
    const alignSelectedElements = useCallback(
      (kind: CanvasAlignKind) => {
        ...
        updateSelectedObjects((obj) => {
          ...
          if (kind === "left")
            return { ...obj, x: obj.x + (bounds.left - box.left) };
          if (kind === "right")
            return {
              ...obj,
          ...
          if (kind === "top")
            return { ...obj, y: obj.y + (bounds.top - box.top) };
          if (kind === "bottom")
            return {
              ...obj,
    ...
    const onZoomSliderChange = useCallback((pct: number) => {
      ...
      if (viewport) {
        ...
        setZoomPercent((prev) => {
          const oldScale = (prev ?? 100) / 100;
          const newScale = clamped / 100;
          setPanX((prevPan) => cx - ((cx - prevPan) / oldScale) * newScale);
          setPanY((prevPan) => cy - ((cy - prevPan) / oldScale) * newScale);
          return clamped;
        });
      } else {
    ...
    const onViewportWheel = useCallback((e: WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        setZoomPercent((prev) => {
          const oldPct = prev ?? 100;
          const oldScale = oldPct / 100;
          const newPct = Math.max(
            ZOOM_MIN_PCT,
            Math.min(ZOOM_MAX_PCT, oldPct * factor)
          );
          const newScale = newPct / 100;
          setPanX(
            (prevPan) => cursorX - ((cursorX - prevPan) / oldScale) * newScale
          );
          setPanY(
            (prevPan) => cursorY - ((cursorY - prevPan) / oldScale) * newScale
          );
          return newPct;
        });
        zoomUserAdjustedRef.current = true;
    ...
    const startWindowDrag = useCallback(
      (state: DragState) => {
        ...
        const onMove = (e: PointerEvent) => {
          ...
          const current = getHandleLocalPosition(
            drag.handle,
            initial.width,
            initial.height
          );
      ...
      <EditorStoreProvider store={editorStore}>
        <VectorBoardControlsProvider value={vectorBoardControls}>
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            <div
              ref={viewportRef}
              className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl bg-[var(--surface-subtle)]"
            ...
            <AiControllerProvider controller={aiController}>
              <EditorSidePanels
                ...
                onSelectPanel={(id) =>
                  setEditorSidebarPanel((prev) => (prev === id ? null : id))
                }
                ready={ready}
              />
