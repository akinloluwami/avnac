...
const SceneEditor = forwardRef<SceneEditorHandle, SceneEditorProps>(
  function SceneEditor(
    ...
    const updateActivePage = useCallback(
      (
        updater: (
          page: import("../lib/avnac-scene").AvnacPage,
        ) => import("../lib/avnac-scene").AvnacPage,
      ) => {
        setDoc((prev) => ({
          ...prev,
          pages: prev.pages.map((p) =>
            p.id === activePageId ? updater(p) : p,
          ),
        }));
      },
      [activePageId, setDoc],
    ...
    const addObjects = useCallback(
      (objectsToAdd: SceneObject[]) => {
        updateActivePage((p) => ({
          ...p,
          objects: [...p.objects, ...objectsToAdd],
        }));
    ...
    const groupSelection = useCallback(() => {
      const picked = activePage.objects.filter((obj) =>
        selectedIds.includes(obj.id),
      );
      const group = createGroupFromSelection(picked);
      if (!group) return;
      const firstIndex = activePage.objects.findIndex((obj) =>
        selectedIds.includes(obj.id),
      );
      const remaining = activePage.objects.filter(
    ...
    const applyBackgroundPicked = useCallback(
      (bg: BgValue) => {
    ...
    const onZoomSliderChange = useCallback((pct: number) => {
      ...
      setZoomPercent(
        Math.max(ZOOM_MIN_PCT, Math.min(ZOOM_MAX_PCT, Math.round(pct))),
      );
    }, []);
    ...
    const startWindowDrag = useCallback(
      ...
      [artboardH, artboardW, pointerToScene, updateActivePage],
    );
    ...
    const onObjectPointerDown = useCallback(
      (e: ReactPointerEvent<HTMLDivElement>, obj: SceneObject) => {
        ...
        let ids = sourceIds;
        let movingObjects = activePage.objects.filter((row) =>
          ids.includes(row.id),
        );
